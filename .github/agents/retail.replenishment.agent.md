...
2. **Constitution Compliance** — confirm Python + MAF/Copilot SDK backend approach (II), harness coverage (III), any UI implications (IV), auditability of recommendations and overrides (V), and cost/Azure WAF notes (VI).
3. **Deliverables Produced** — updated artifacts (requirements, explanation schema) with file paths.
4. **Harness Coverage** — scenarios covering policy and lead-time variations, and override traceability.
